	...
	*/
	...
	class woocommerce_paypal_payments_advanced extends WC_Payment_Gateway {
		...
		function init_form_fields() {
			...
			'avs' => array(
							'title' => __( 'AVS', 'woocommerce' ), 
							'label' => __( 'Require billing address verification to match the card billing address', 'woocommerce' ), 
							'type' => 'checkbox', 
...
// 							'description' => __( 'Enable test mode. This causes the gateway to simulate payments through PayPal\'s development server.', 'woocommerce' ),
		...
		function process_ipn_success() {
			...
			if (
				...
				$this->process_ipn_response( true );
				
			$thanks_page = woocommerce_get_page_id( 'thanks' );
			
			if ( isset( $thanks_page ) && absint( $thanks_page ) > 0 ) {
			
				$thanks_page = get_permalink( $thanks_page );
			
				echo '<script>top.location="' . $thanks_page . '"</script>';
		...
		function process_ipn_response( $do_return = false ) {
		...
		function generate_paypal_form( $order_id ) {
			...
			// Make the site URL protocol relative by remove protocol prefix
			$ssl = get_option( 'woocommerce_force_ssl_checkout', false );
			...
			$txn['CURRENCY'] = get_woocommerce_currency();
			...
			$txn['BUTTONSOURCE'] = 'IgniteWoo_SI_Custom';
			...
			return '<iframe id="payments_advanced_id" width="490" height="500" border="0" frameborder="0" scrolling="no" src="" >
				...
				</iframe><div style="clear:both; margin: 50px 0"></div>
				';
