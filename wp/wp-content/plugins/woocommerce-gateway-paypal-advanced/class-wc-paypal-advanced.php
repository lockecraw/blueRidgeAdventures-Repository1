...
class WC_Paypal_Advanced extends WC_Payment_Gateway {
	...
	public function checks() {
		...
		if ( ! $this->loginid ) {
			...
			echo '<div class="error"><p>' . sprintf( __('Paypal Advanced error: Please enter your PayPal Advanced Account Merchant Login <a href="%s">here</a>', 'wc_paypaladv'), admin_url('admin.php?page=woocommerce&tab=payment_gateways&subtab=gateway-paypal_advanced' ) ) . '</p></div>';

		} elseif ( ! $this->resellerid ) {

			echo '<div class="error"><p>' . sprintf( __('Paypal Advanced error: Please enter your PayPal Advanced Account Partner <a href="%s">here</a>', 'wc_paypaladv'), admin_url('admin.php?page=woocommerce&tab=payment_gateways&subtab=gateway-paypal_advanced' ) ) . '</p></div>';

		} elseif ( ! $this->password ) {

			echo '<div class="error"><p>' . sprintf( __('Paypal Advanced error: Please enter your PayPal Advanced Account Password <a href="%s">here</a>', 'wc_paypaladv'), admin_url('admin.php?page=woocommerce&tab=payment_gateways&subtab=gateway-paypal_advanced' ) ) . '</p></div>';
		}
	...
	public function relay_response() {
		...
		if ( $_REQUEST['error'] == 'true' && $_POST['RESULT'] != 0 ) { //handle errors and declines
			...
			// Add error
			$woocommerce->add_error( __('Error:', 'wc_paypaladv') . ' "' . urldecode($_POST['RESPMSG']) . '"' );
		...
		}elseif($_REQUEST['cancel_ec_trans']=='true' && !isset($_REQUEST['silent'])){//handle cancellations
...
/**
	...
	function get_secure_token( $order, $limited_params= false ) {
		...
		if ( $order->prices_include_tax=='yes' || $order->get_order_discount() > 0 ) {
			...
			if ( sizeof( $order->get_items() ) > 0 )
				foreach ( $order->get_items() as $item )
					...
					$paypal_args['L_COST1']   = number_format($order->get_total() - $order->get_shipping() - $order->get_shipping_tax() + $order->get_order_discount(), 2, '.', '');
				...
				if ( ( $order->get_shipping() + $order->get_shipping_tax() ) > 0 ) :
					...
					$paypal_args['L_COST2'] 	= number_format( $order->get_shipping() + $order->get_shipping_tax() , 2, '.', '' );
				endif;
			...
			if ($order->get_shipping() + $order->get_shipping_tax() >0) {
				...
				$paypal_args['L_COST'.$item_loop] = $order->get_shipping();
		...
		try {
			...
			if ($this->debug=='yes') $this->log->add( 'paypal_advanced', sprintf(__('Secured Token generation failed for the order #%s with error: %s', 'wc_paypaladv'), $order_id, $e->getMessage() ) );
	
			if($arr['RESULT'] != 7){
				$woocommerce->add_error( __('Error:', 'wc_paypaladv') . ' "' . $e->getMessage() . '"' );
				return;
			} else {
